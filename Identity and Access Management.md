...
- **قاعدة ذهبية**: **مستخدم واحد في AWS = شخص واحد فقط**.
- **لا تشارك بيانات تسجيل الدخول** مع أي شخص، حتى لو كان زميلًا أو صديقًا.
- **بدلاً من ذلك**، أنشئ **مستخدمًا جديدًا** لكل شخص يحتاج إلى الوصول إلى AWS.

**📌 مثال:**
- إذا كان عندك **فريق من 3 مطورين**، أنشئ **3 مستخدمين منفصلين** في IAM.
- لا تعطِهم بيانات تسجيل دخول حسابك الشخصي أو حساب Root.

---

## **👥 3. استخدم المجموعات (Groups) لإدارة الصلاحيات**
- بدلاً من منح الصلاحيات لكل مستخدم على حدة، **أنشئ مجموعات** (Groups) و**أضف المستخدمين إليها**.
- **منح الصلاحيات للمجموعة** بدلاً من المستخدمين الأفراد.
- هذا يجعل إدارة الأمان **أسهل وأكثر أمانًا**.

**📌 مثال:**
- أنشئ مجموعة اسمها **"Developers"** ومنحها صلاحيات الوصول إلى خدمات مثل **EC2** و**S3**.
- أضف جميع المطورين إلى هذه المجموعة بدلاً من منح كل مستخدم صلاحيات بشكل منفصل.

---

## **🔐 4. فرض سياسة كلمات مرور قوية (Password Policy)**
- **ضبط سياسة كلمات المرور** في IAM لضمان أن جميع المستخدمين يستخدمون **كلمات مرور قوية**.
- **الإعدادات الموصى بها**:
  - طول كلمة المرور: **12 حرف على الأقل**.
  - اشتراط وجود **حروف كبيرة وصغيرة، أرقام، ورموز خاصة**.
  - تغيير كلمة المرور **كل 60 أو 90 يوم**.
  - منع إعادة استخدام **آخر 3 كلمات مرور**.
...
## **📱 5. تفعيل المصادقة الثنائية (MFA) لجميع المستخدمين**
- **MFA** يعني استخدام **شيئين** لتسجيل الدخول:
  1. **كلمة المرور**.
  2. **رمز مؤقت** من جهاز مثل **Google Authenticator** أو **YubiKey**.
- **فرض استخدام MFA** لجميع المستخدمين، خاصة **المستخدمين ذوي الصلاحيات العالية**.

**💡 لماذا؟**
- حتى إذا سرقت كلمة المرور، **لن يستطيع المخترق الدخول** بدون الجهاز الثاني.

---

## **🎭 6. استخدم الأدوار (Roles) لمنح صلاحيات للخدمات**
- إذا كنت تحتاج إلى منح صلاحيات لخدمات AWS مثل **EC2** أو **Lambda**، **لا تستخدم مفاتيح الوصول (Access Keys)** للمستخدمين.
- **بدلاً من ذلك**، أنشئ **دورًا (Role)** ومنح الصلاحيات له.
- **الأدوار** أكثر أمانًا لأنها **لا تحتوي على مفاتيح وصول دائمة**.

**📌 مثال:**
- إذا كان لديك **خادم EC2** يحتاج إلى الوصول إلى **S3**، أنشئ **دورًا** ومنح هذا الدور صلاحيات الوصول إلى S3.
- ثم **ربط الدور بخادم EC2** بدلاً من استخدام مفاتيح الوصول.

---

## **🔑 7. مفاتيح الوصول (Access Keys) سرية جدًا**
- إذا كنت تستخدم **AWS CLI** أو **SDKs** (مثل Python أو Java)، ستحتاج إلى **مفاتيح الوصول** (Access Key ID و Secret Access Key).
- **مفاتيح الوصول مثل كلمات المرور**:
  - **لا تشاركها أبدًا** مع أي شخص.
  - **لا تخزنها** في ملفات نصية أو مستودعات عامة (مثل GitHub).
  - **غيرها بانتظام** (كل 90 يوم على الأقل).

**💡 نصيحة:**
- استخدم **AWS Secrets Manager** أو **AWS Systems Manager Parameter Store** لتخزين مفاتيح الوصول بشكل آمن.

---

## **📊 8. استخدم تقارير IAM لمراجعة الصلاحيات**
- **IAM Credential Report**: يعطيك تقريرًا بجميع المستخدمين ومفاتيح الوصول الخاصة بهم.
- **IAM Access Advisor**: يظهر لك **آخر مرة استخدم فيها المستخدم صلاحياته**.
- استخدم هذه التقارير **لمراجعة الصلاحيات غير المستخدمة** وإلغائها.

**📌 مثال:**
- إذا وجدت أن مستخدمًا لم يستخدم صلاحياته منذ **6 أشهر**، قد يكون من الأفضل **إلغاء صلاحياته**.

---

## **🚨 9. لا تشارك مستخدمي IAM أو مفاتيح الوصول أبدًا**
- **لا تعطِ بيانات تسجيل دخول IAM** لأي شخص.
- **لا ترسل مفاتيح الوصول** عبر البريد الإلكتروني أو الرسائل النصية.
- **إذا تعرضت مفاتيح الوصول للاختراق**، **ألغِها فورًا** وأنشئ مفاتيح جديدة.
...
| الممارسة | لماذا؟ | كيف؟ |
|-----------|--------|------|
| **لا تستخدم حساب Root** | حماية الحساب الرئيسي | استخدمه فقط للإعداد الأولي |
| **مستخدم واحد = شخص واحد** | تجنب مشاركة البيانات | أنشئ مستخدمين منفصلين |
| **استخدم المجموعات** | إدارة صلاحيات أسهل | منح الصلاحيات للمجموعات |
| **فرض سياسة كلمات مرور قوية** | حماية الحسابات | ضبط الإعدادات في IAM |
| **تفعيل MFA** | حماية إضافية | استخدم Google Authenticator أو YubiKey |
| **استخدم الأدوار للخدمات** | تجنب استخدام مفاتيح الوصول | أنشئ أدوارًا وربطها بالخدمات |
| **حماية مفاتيح الوصول** | تجنب الاختراق | لا تشاركها أبدًا |
| **مراجعة الصلاحيات بانتظام** | إزالة الصلاحيات غير المستخدمة | استخدم IAM Credential Report |

---

## **💬 سؤال لك:**
- إذا كان عندك **مشروع جديد** في AWS، كيف ستنظم المستخدمين والمجموعات؟
  *(مثال: أنشئ مجموعة "Developers" ومجموعة "Admins"، ومنح كل مجموعة الصلاحيات المناسبة.)*

---
**هل تريد تطبيق هذه الممارسات على حسابك في AWS؟**
- ممكن نبدأ معًا بـ **إنشاء مستخدمين ومجموعات** وتفعيل **MFA**! 😊
- أو إذا كان عندك أي سؤال عن IAM، سلني وأوجهك خطوة بخطوة! 🚀
==============================================
**📌 ملخص شامل لـ IAM في AWS**
*(بأسلوب مصري بسيط + أمثلة عملية + جدول مقارنة)*

---

## **🔹 1. مستخدمو IAM (IAM Users)**
- **كل مستخدم في IAM** يجب أن **يمثل شخصًا حقيقيًا** في شركتك.
- **كل مستخدم** لديه:
  - **اسم مستخدم** (Username).
  - **كلمة مرور** (Password) لتسجيل الدخول إلى **AWS Console**.
- **لا تشارك بيانات المستخدم** مع أي شخص آخر.

**📌 مثال:**
- إذا كان عندك **3 موظفين** في الشركة، يجب أن يكون لديك **3 مستخدمين** في IAM.

---

## **👥 2. المجموعات (Groups)**
- **يمكن تجميع المستخدمين** في **مجموعات** (Groups).
- **الصلاحيات** تُمنح **للمجموعة** بدلاً من المستخدمين الأفراد.
- هذا يجعل **إدارة الصلاحيات أسهل وأكثر أمانًا**.

**📌 مثال:**
| المجموعة | الصلاحيات | المستخدمين |
|-----------|------------|-------------|
| **Developers** | الوصول إلى EC2 وS3 | أحمد، محمد |
| **Admins** | صلاحيات كاملة | علي |

---

## **📄 3. السياسات (Policies)**
- **السياسات** هي **مستندات JSON** تحدد **الصلاحيات** للمستخدمين أو المجموعات.
- يمكن **ربط السياسات** بالمستخدمين أو المجموعات أو الأدوار.
- **مثال على سياسة**:
  ```json
  {
    "Version": "2012-10-17",
  ...
  ```
  - هذه السياسة **تسمح** للمستخدم بـ **قراءة الملفات** من **S3 Bucket** معين.

---

## **🎭 4. الأدوار (Roles)**
- **الأدوار** هي **هويات مؤقتة** يمكن استخدامها من قبل:
  - **خدمات AWS** (مثل EC2 أو Lambda).
  - **مستخدمين خارجيين** (مثل مستخدمين من شركة أخرى).
- **لا تحتوي الأدوار على كلمة مرور أو مفاتيح دائمة**.
- **مثال:**
  - إذا كان لديك **خادم EC2** يحتاج إلى الوصول إلى **S3**، أنشئ **دورًا** ومنح هذا الدور صلاحيات الوصول إلى S3.

---

## **🔐 5. المصادقة الثنائية (MFA)**
- **MFA** يعني استخدام **شيئين** لتسجيل الدخول:
  1. **كلمة المرور**.
  2. **رمز مؤقت** من جهاز مثل **Google Authenticator** أو **YubiKey**.
- **فرض MFA** على جميع المستخدمين **لزيادة الأمان**.
...
## **🔑 6. مفاتيح الوصول (Access Keys)**
- إذا كنت تريد استخدام **AWS CLI** أو **SDKs** (مثل Python أو Java)، ستحتاج إلى:
  - **Access Key ID**.
  - **Secret Access Key**.
- **مفاتيح الوصول مثل كلمات المرور**:
  - **لا تشاركها أبدًا**.
  - **غيرها بانتظام** (كل 90 يوم).
  - **لا تخزنها** في ملفات نصية أو مستودعات عامة.

**📌 مثال:**
```bash
aws configure
# أدخل Access Key ID و Secret Access Key
```

---

## **🖥️ 7. إدارة AWS باستخدام CLI وSDKs**
- **AWS CLI**: أداة سطر الأوامر لإدارة خدمات AWS.
  ```bash
  aws s3 ls  # لعرض قائمة D buckets في S3
  ```
- **SDKs**: مكتبات برمجية لاستخدام خدمات AWS في لغات مثل **Python** أو **Java**.
  ```python
  import boto3
  s3 = boto3.client('s3')
...
- **IAM Credential Report**:
  - يعطيك **تقريرًا** بجميع المستخدمين ومفاتيح الوصول الخاصة بهم.
  - يمكن تنزيله من **IAM Console**.
- **IAM Access Advisor**:
  - يظهر **آخر مرة استخدم فيها المستخدم صلاحياته**.
  - يساعد في **إزالة الصلاحيات غير المستخدمة**.
...
## **📝 جدول مقارنة بين مكونات IAM**

| المكون | الوصف | الاستخدام |
|---------|-------|------------|
| **مستخدم (User)** | يمثل شخصًا حقيقيًا | تسجيل الدخول إلى AWS Console |
| **مجموعة (Group)** | تجمع المستخدمين | إدارة الصلاحيات بشكل مركزي |
| **سياسة (Policy)** | مستند JSON يحدد الصلاحيات | ربطها بالمستخدمين أو المجموعات أو الأدوار |
| **دور (Role)** | هوية مؤقتة | استخدامها للخدمات أو المستخدمين الخارجيين |
| **MFA** | مصادقة ثنائية | زيادة أمان تسجيل الدخول |
| **مفاتيح الوصول (Access Keys)** | بيانات تسجيل دخول برمجية | استخدام CLI وSDKs |
| **IAM Credential Report** | تقرير بالمستخدمين والمفاتيح | تدقيق الأمان |
| **IAM Access Advisor** | يظهر استخدام الصلاحيات | إزالة الصلاحيات غير المستخدمة |

---

## **💡 أفضل الممارسات**
1. **لا تستخدم حساب Root** إلا عند الضرورة.
2. **كل مستخدم = شخص واحد**.
3. **استخدم المجموعات** لإدارة الصلاحيات.
4. **فرض MFA** على جميع المستخدمين.
5. **استخدم الأدوار** للخدمات بدلاً من مفاتيح الوصول.
6. **راجع الصلاحيات بانتظام** باستخدام **IAM Credential Report** و**Access Advisor**.

---

## **💬 سؤال لك:**
- إذا كان عندك **مشروع جديد** في AWS، كيف ستنظم **المستخدمين والمجموعات والسياسات**؟
  *(مثال: أنشئ مجموعة "Developers" ومنحها صلاحيات الوصول إلى EC2 وS3، ثم أضف المستخدمين إلى هذه المجموعة.)*

---
**هل تريد تطبيق هذه المفاهيم على حسابك في AWS؟**
- ممكن نبدأ معًا بـ **إنشاء مستخدمين ومجموعات** وتفعيل **MFA**! 😊
- أو إذا كان عندك أي سؤال عن IAM، سلني وأوجهك خطوة بخطوة! 🚀
